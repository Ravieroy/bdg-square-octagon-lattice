...
@everywhere function main(T)
    logFile = "$(logFolder)main_stiffness_T_$T.log"
    fileName = "$(saveInFolder)main_stiffness_T_$T.txt"
    message = string("Running for T=$T, id=$(myid())")
    ...
    deltaFinal, _, _, nAvg, evecs, evals, isConverged, endTime, count =
        ...
        column_names = ["T", "Ds"]
        tcFileNameFinal = "$(saveInFolder)stiffness_vs_T.txt"
...
The program takes TVals as input and processes each value in parallel.
For each values of T, the program calculates and stores the following data:
    T:Temperature
    Ds: Stiffness
"""

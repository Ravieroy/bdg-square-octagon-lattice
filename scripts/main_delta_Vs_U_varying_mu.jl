...
#   values of on-site interaction U and μ
#
...
#   it will be slower if only one calculation for μ is needed. It is better to
...
#   See paper PRA. 97, 053619 (2022) by Agnieszka Cichy et.al(Fig. 3)
#
using Distributed
@everywhere begin
...
@everywhere function main(μ)
    ...
    for U in UList
        st = time()
        message = string("Running for μ=$μ, U=$U")
        write_log(logFileName, message)
        ...
        et = time() - st
        message = string("Completed for μ=$μ, U=$U in ", format_elapsed_time(et))
        write_log(logFileName, message)
    end
    GC.gc()
end  # main end

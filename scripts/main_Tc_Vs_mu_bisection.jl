...
@everywhere function main(μ)
    ...
    fileName = "$(saveInFolder)Tc_mu_$μ.txt"
    write_log(logFileName, "Running for mu = $μ, id = $(myid())")
    # === CSV logging setup ===
    csvfile = "$(logFolder)Tc_calculation_log_$μ.csv"
...
desc = """
Calculates the critical temperature Tc for various chemical potential μ.

module GeneralUtils
    ...
    """
        check_rel_tol(oldList, newList; tol::Real=1e-5, abs_tol::Real=1e-7, nRound::Int=10)
    ...
    """
    function check_rel_tol(oldList, newList; tol::Real=1e-5, abs_tol::Real=1e-7, nRound::Int=10)
    ...
    """
        is_hermitian(matrix; tol=1e-10)

    Return `(is_Hermitian, max_deviation)` where
    - `is_Hermitian` is true if `‖matrix – matrix†‖_∞ < tol`,

    Throws an error if `matrix` is not square.
    """
    function is_hermitian(matrix::AbstractMatrix{T}; tol::Real = 1e-10) where T<:Number
        size(matrix, 1) == size(matrix, 2) ||
            throw(ArgumentError("Matrix must be square; got size=$(size(matrix))"))
        maxdev = maximum(abs.(matrix .- adjoint(matrix)))
        return maxdev < tol
    end
